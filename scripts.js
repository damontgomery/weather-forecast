...
const zoomOutScreenHandler = () => {
    ...
    while (projectedLastForecastedPoint > forecastPoints.length || projectedFirstForecastedPoint < 0) {
        selectedScreen--;
        projectForecastedPoints();
    }
    drawSVG();
...
const zoomInScreenHandler = () => {
    ...
    selectedForecastPointsPerScreen = Math.max(2, Math.floor(selectedForecastPointsPerScreen / 2));
    drawSVG();
};
const drawSVG = () => {
    const selectedForecastPoints = forecastPoints.slice(selectedScreen * selectedForecastPointsPerScreen, (selectedScreen + 1) * selectedForecastPointsPerScreen);
    ...
    const startTime = new Date(selectedForecastPoints[0].startTime);
    ...
    const endTime = new Date(selectedForecastPoints[selectedForecastPoints.length - 1].endTime);
    ...
    // Draw time gridlines.
    selectedForecastPoints.forEach((forecastPoint, i) => {
        const startTime = new Date(forecastPoint.startTime);
        let labelGap = Math.ceil(selectedForecastPoints.length / 8);
        ...
        const shouldDrawLabel = (i == 0 ||
            i == selectedForecastPoints.length - 1 ||
            startTime.getHours() % labelGap == 0);
        if (!shouldDrawLabel) {
            return;
        }
        const x = temperatureCanvasBounds.x.min + temperatureCanvasBounds.x.length * (i / (selectedForecastPoints.length - 1));
    ...
    const temperatures = [
        ...selectedForecastPoints.map(forecastPoint => forecastPoint.temperature),
        ...selectedForecastPoints.map(forecastPoint => forecastPoint.dewPoint),
        ...selectedForecastPoints.map(forecastPoint => forecastPoint.windChill),
    ].filter(n => !isNaN(n));
    ...
    const drawTemperatureLine = ({ propertyName, className }) => {
        drawPolyline({
            points: selectedForecastPoints.map((forecastPoint, i) => ({
                x: temperatureCanvasBounds.x.min + temperatureCanvasBounds.x.length * (i / (selectedForecastPoints.length - 1)),
                // @ts-ignore
    ...
    const drawPrecipitationLine = ({ propertyName, className }) => {
        drawPolyline({
            points: selectedForecastPoints.map((forecastPoint, i) => {
                ...
                return {
                    x: precipitationCanvasBounds.x.min + precipitationCanvasBounds.x.length * (i / (selectedForecastPoints.length - 1)),
...
const fetchWeatherForecastData = () => {
    const xhr = new XMLHttpRequest();
    const promise = new Promise((resolve, reject) => {
        xhr.onload = () => {
            forecastPoints = getForecastPointsFromXML();
            resolve(forecastPoints);
        };
    });
